...
function M.join_spaceless()
    ...
    if char:match('%s') ~= nil then vim.api.nvim_exec2("normal dw<cr>", {}) end
end
...
function M.onlyAndNeotree()
    ...
    for _, windowId in pairs(vim.api.nvim_tabpage_list_wins(0)) do
        if windowId ~= currentWindowId and
            vim.api.nvim_get_option_value("filetype", {win = windowId}) ~=
            "neo-tree" then
            vim.api.nvim_win_call(windowId,
                                  function() vim.cmd("silent! close") end)
        end
    end
end
