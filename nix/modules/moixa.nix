  ...
  environment.systemPackages =
    ...
    let
      ...
      base64Decode = pkgs.writeShellScriptBin "base_64_decode" ''
        ${pkgs.nodejs}/bin/node -e "console.log(Buffer.from(process.argv[1], 'base64').toString('utf8'))" -- "$@"
      '';
      base64Encode = pkgs.writeShellScriptBin "base_64_encode" ''
        ${pkgs.nodejs}/bin/node -e "console.log(Buffer.from(process.argv[1], 'utf8').toString('base64'))" -- "$@"
      '';
    in
    [
      unstable.ngrok
